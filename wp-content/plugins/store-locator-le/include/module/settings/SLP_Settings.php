 ...
 *
 */
class SLP_Settings extends SLP_Base_ReactObject {
	...
	function __construct( $options = array() ) {
		...
		// phpcs:ignore we won't always have a nonce we control when first rendering a plugin page
		// phpcs:ignore
		$this->current_admin_page = ! empty( $_GET['page'] ) ? plugin_basename( stripslashes( sanitize_key( $_GET['page'] ) ) ) : '';
	...
	private function add_help_section() {
		...
		$helpHeaderText    = $this->slplus->Text->get_text_string( array( 'admin', 'help_header' ) );
		$helpSubheaderText = $this->slplus->Text->get_text_string( array( 'admin', 'help_subheader' ) );

		$this->enqueue_help_script();
		?>
        <section class="dashboard-aside-secondary">
            <h3 class="panel-content aside-heading"><?php echo wp_kses( $helpHeaderText, 'strip' ); ?></h3>
            <div class="panel-content">
                <h4 class="description-heading"><?php echo wp_kses( $helpSubheaderText, 'strip' ); ?></h4>
	 ...
	 */
	public function add_ItemToGroup( $params, $option = null ) {
	...
	function render_settings_page() {
		...
		$form_action         = esc_attr( $this->form_action );
		$form_id             = esc_attr( $this->form_name );
		$form_name           = esc_attr( $this->form_name );
		$form_enc            = esc_attr( $this->form_enctype );
		$documentation_label = $this->slplus->Text->get_text_string( array( 'admin', 'Documentation' ) );
		$documentationLink   = $this->slplus->Text->get_url( 'slp_docs' );
            ...
            <div class='dashboard-main store-locator-plus'>
                <section class='dashboard-content'>
                    <div id='wpcsl_container'
                         class='settings_page page_<?php echo esc_attr( $this->current_admin_page ) ?>'>
                        <form method='post'
                              class='slplus_settings_form'
                              action='<?php echo $form_action ?>'
                              id='<?php echo $form_id ?>'
                              name='<?php echo $form_name ?>'
                              enctype='<?php echo $form_enc ?>'
                        >
                            <input type='hidden' id='selected_nav_element' name='selected_nav_element'
                                   value='<?php echo esc_attr( $selectedNav ); ?>'>
							<?php
							wp_nonce_field( 'slp_ajax', 'security' );
                            ...
                            <div id="main" class="dashboard-content-inner panel-settings">
                                ...
                                <div id="wpcsl-nav" class="sub_navigation">
                                    <ul class="sub-navbar">
										...
										foreach ( $this->sections as $section ) {
											if ( $section->auto ) {
												$div_id     = esc_attr( ! empty( $section->div_id ) ? $section->div_id : $section->slug );
												$link_id    = esc_attr( "wpcsl-option-{$div_id}" );
												$firstClass = esc_attr( ( ( "#{$link_id}" == $selectedNav ) || ( $firstOne && ( $selectedNav == '' ) ) ) ? ' first current open' : '' );
												$title      = esc_attr( $section->name );
												$titleText  = wp_kses_post( $section->name );
												$firstOne   = false;
												?>
                                                <li class='top-level general {$firstClass} navbar-item'>
                                                    <a id='<?php echo $link_id ?>_sidemenu'
                                                       class='navbar-link subtab_link'
                                                       data-slug='<?php echo $div_id ?>'
                                                       href='#<?php echo $link_id ?>'
                                                       title='<?php echo $title ?>'><?php echo $titleText ?></a>
                                                </li>
												<?php
											}
										}
	...
	private function save_button() {
		...
		?>
        <li class='top-level general save-button'>
            <div class="navsave">
                <input type="submit" class="button-primary"
                       value="<?php echo esc_attr( $this->save_text ); ?>"/>
            </div>
        </li>
		<?php
	}
