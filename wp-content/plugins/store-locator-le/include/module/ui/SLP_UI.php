...
class SLP_UI extends SLPlus_BaseClass_Object {
	...
	private $shortcode_map_region = 'us';
	private $js_attributes = array();
	...
	protected function initialize() {
		...
		add_shortcode( 'SLPlus', array( $this, 'render_shortcode' ) );
		add_shortcode( 'SLplus', array( $this, 'render_shortcode' ) );
		$this->register_locations_map_block_type();
	}

	/**
	 * Register Locations Map Block Type
	 */
	private function register_locations_map_block_type() {
		register_block_type( 'store-locator-plus/basic-locations-map',
			array(
				'render_callback' => array( $this, 'render_slp_block' )
			)
		);
	}
	 ...
	 * @return string
	 */
	public function render_slp_block( $attributes ) {
	 ...
	 */
	public function createstring_SearchForm() {
		...
		if ( empty( $search_dom ) || ( strpos( $search_dom, '<form id="searchForm"' ) !== false ) ) {
			return $search_dom;
		}
	...
	public function modify_searchlayout( $html ) {
		...
		// Hide Address Input
		//
		if ( $this->slplus->SmartOptions->hide_address_entry->is_true ) {
			$html = preg_replace( '/\[slp_search_element\s+(\s\w)*input_with_label="address".*?\]/i', '', $html );
		}
	 ...
	 */
	public function filter_SetDefaultMapLayout( $HTML ) {
		if ( ! empty( $HTML ) ) {
			return $HTML;
		}
	...
	public function create_string_radius_selector_div() {
		if ( $this->slplus->SmartOptions->hide_radius_selector->is_true ) {
			$HTML = "<input type='hidden' id='radiusSelect' name='radiusSelect' value='" . $this->find_default_radius() . "'>";
		} else {
	...
	private function get_find_button_text() {
		 ...
		 */
		return apply_filters( 'slp_find_button_text', SLP_Text::get_instance()->get_text_string( array(
			'option_default',
	 ...
	 */
	public function create_MapContent() {
		if ( empty( $this->slplus->SmartOptions->google_server_key->value ) ) {
			return '<div class="slp_map missing_api_key">' .
			       '<p>' .
			       __( 'The Google Maps API key in Store Locator Plus® has not been set. ', 'store-locator-le' ) .
			       sprintf( __( 'Please <a href="%s">add one</a> or use the <a href="%s">Store Locator Plus® SaaS Edition</a> to avoid the Google API license fees and headaches.', 'store-locator-le' ), $this->slplus->support_url . '/getting-started/', $this->slplus->url_main_slp_site ) .
			       '</p>' .
			       '</div>';
		}

		return '<div id="map" class="slp_map"></div>';
	}
	...
	static function no_texturize_shortcodes( $shortcodes ) {
		return array_merge( $shortcodes, array( 'STORE-LOCATOR', 'SLPLUS', 'slplus', 'SLPlus', 'SLplus' ) );
	}
	...
	public function render_shortcode( $attributes, $content = null ) {
		...
		$this->shortcode_map_region = SLP_Country_Manager::get_instance()->countries[ SLP_SmartOptions::get_instance()->default_country->value ]->cctld;
		...
		if ( ! empty( $attributes ) ) {
			...
			foreach ( $attributes_used as $key => $value ) {
				...
				// Drop lat/lng if center map specified
				if ( ( $key === 'center_map_at' ) || ( $key === 'map_center' ) ) {
					$this->slplus->options['map_center_lat'] = '';
					$this->slplus->options['map_center_lng'] = '';
				}

				if ( $key === 'map_region' ) {
					$this->shortcode_map_region = $value;
				}
			}
		...
		//
		SLP_UI_Shortcode_slp_option::get_instance()->activate();
		 ...
		 */
		if ( ! empty( $attributes['hide_until_called'] ) ) {
			$style = ' hidden ';
		} else {
			$style = '';
		}
		$HTML = '<div class="store_locator_plus ' . sanitize_title( $current_theme->get_template() ) . $style . '">' .
	...
	public function localize_script() {
		...
		//
		$this->slplus->SmartOptions->set( 'map_center', $this->set_MapCenter() );

		add_filter( 'slp_js_options', array( $this, 'add_to_js_options' ) );  // Extend our JS data
		add_filter( 'slp_js_options', array(
			$this,
			'strip_slashes_from_smart_options'
		), 999 );  // Strip slashes from Text Smart Options
		...
		// Look for changed data via widgets etc.
		...
		$scriptData['nonce']      = wp_create_nonce( 'wp_rest' );
		$scriptData['apikey']     = SLPlus::get_instance()->get_apikey();
		$scriptData['rest_url']   = rest_url( 'store-locator-plus/v2/' );

		// Messages
		$scriptData['messages'] = $this->get_messages();

		// Shortcode atts
		$scriptData['shortcode_attributes'] = $this->js_attributes;
	...
	public function add_to_js_options( $options ) {
		return array_merge( $options, array(
			'map_region'    => $this->shortcode_map_region,
			'resultslayout' => $this->set_ResultsLayout( false, true ),
			'use_sensor'    => property_exists( $this->slplus->SmartOptions, 'use_sensor' ) && $this->slplus->SmartOptions->use_sensor->is_true ? true : false,
		) );
	}
	...
	private function assign_user_stylesheet( $themeFile ) {
		...
		if ( is_readable( SLPLUS_PLUGINDIR . 'css/' . $themeFile ) ) {
			...
			wp_enqueue_style( SLPLUS_PREFIX . '_user_header_css', SLPLUS_PLUGINURL . '/css/' . $themeFile, array(), SLPLUS_VERSION );
		}
	...
	function set_MapCenter() {
		...
		if ( ( preg_replace( '/\W/', '', $customAddress ) != '' ) ) {
			...
			$customAddress = esc_attr( SLP_Country_Manager::get_instance()->countries[ SLP_SmartOptions::get_instance()->default_country->value ] );
		}

		$new_address = apply_filters( 'slp_map_center', $customAddress );

		// If the address is not the default, clear out lat/lng so it will be forced to be looked up.
		//
		if ( $new_address !== $this->slplus->SmartOptions->map_center->value ) {
			$this->slplus->SmartOptions->set( 'map_center_lng', null );
			$this->slplus->options['map_center_lng'] = null;

			$this->slplus->SmartOptions->set( 'map_center_lat', null );
