...
if ( ! class_exists( 'SLP_SmartOptions' ) ) {
	 ...
	 *
	 * The Smart Options is an object that defines the user settings for Store Locator Plus.
	 * It helps define properties that are used to render the administrative UI, default values,
	 * and the behavior of the setting throughout the application.
	 *
	 ...
	 * @property        string $default_country                         The default country.
	 ...
	 *
	 * SLP
	 * @property    SLP_Option $active_style_css
	 * @property    SLP_Option $force_load_js
	 * @property    SLP_Option $hide_address_entry
	 * @property    SLP_Option $hide_radius_selector
	 ...
	 * @property    string $premium_subscription_id
	 * @property    string $premium_user_id
	 *
	 * Experience
	 ...
	 * @property    SLP_Option $hide_radius_selections
	 ...
	 * @property    SLP_Option $no_results_gf_id
	 * @property    SLP_Option $no_results_interface
	 *
	 * Power
	 ...
	 * @property    SLP_Option $permalink_flush_needed
	...
	class SLP_SmartOptions extends SLPlus_BaseClass_Object {
		...
		public $premium_subscription_id;
		public $premium_user_id;
		 ...
		 */
		public function change_map_center() {
			$this->set( 'map_center_lat', null );
			$this->set( 'map_center_lng', null );

			$this->slplus->recenter_map();

			$this->recalculate_initial_distance();
		}
...
ACTIVE_STYLE_CSS
		 ...
		 * Settings
		 */
		private function create_settings_options() {
			$this->experience_search_functionality();
			$this->settings_search_appearance();
			...
			$this->results_appearance_no_results();
		...
		private function create_general_options() {
			...
			$this->general_app_map_services();
			$this->general_app_features();
		 ...
		 *
		 * @used-by \SLP_SmartOptions::save
		 * @used-by \SLP_Admin_General::save_options
		 * @used-by \SLP_Admin_Settings::save_options
		 * @used-by \SLP_AJAX::slp_change_option
		 * @used-by \SLP_SmartOptions::set
		 * @used-by \SLP_UI::localize_script
		 * @used-by \SLP_BaseClass_Activation::convert_to_smartoptions
		 *
		 */
		public function execute_change_callbacks() {
			if ( ! empty( $this->change_callbacks ) ) {
				foreach ( $this->change_callbacks as $callback_info ) {
					if ( $callback_info === $this->current_callback ) {
						continue;
					}
					$this->current_callback = $callback_info;
		...
		private function experience_map_appearance() {
			$smart_options['map_height']       = array(
				'default'    => '480',
				'related_to' => 'map_height,map_height_units',
			);
			...
			$smart_options['map_width']        = array(
				'default'    => '100',
				'related_to' => 'map_width,map_width_units',
			);
			...
			$smart_options['maplayout']        = array(
				...
				'related_to'          => 'maplayout,hide_map,show_maptoggle'
			);
			$smart_options['bubblelayout']     = array(
				'use_in_javascript'   => true,
				'type'                => 'textarea',
				'add_to_settings_tab' => false,
				'related_to'          => 'bubblelayout,bubble_footnote,hide_bubble',
...
BUBBLELAYOUT
			...
			$this->create_smart_options( $smart_options, array(
				'page'    => 'slp_experience',
				'section' => 'map',
				'group'   => 'appearance',
		...
		private function experience_map_functionality() {
			 ...
			 */
			$selections = array();
			foreach ( SLP_Country_Manager::get_instance()->countries as $country_slug => $country ) {
				$selections[] = array(
					'label' => "{$country->name} ({$country->google_domain})",
					'value' => $country_slug,
				);
			}
			$smart_options['default_country'] = array(
				'type'              => 'dropdown',
				'default'           => 'us',
				'use_in_javascript' => false,
				'options'           => $selections
		...
		private function experience_map_markers() {
			$smart_options['map_home_icon'] = array(
				...
				'type'              => 'icon',
				'use_in_javascript' => true,
				'related_to'        => 'map_home_icon,map_end_icon,map_options_clickableIcons',
...
RESULTSLAYOUT
		 ...
		 */
		private function results_appearance_no_results() {
			$smart_options['message_no_results'] = array(
				'is_text'    => true,
				'type'       => 'textarea',
				'related_to' => 'message_no_results,no_results_interface,no_results_gf_id,no_results_gf_landing_page',
			);
			$this->create_smart_options( $smart_options, array(
				'page'    => 'slp_experience',
				'section' => 'results',
				'group'   => 'appearance_no_results',
				'classes' => array( 'quick_save' )
			) );
		 ...
		 */
		private function settings_search_appearance() {
...
SEARCHLAYOUT
			...
			$smart_options['hide_address_entry'] = array(
				'type'       => 'checkbox',
				'default'    => '0',
				'related_to' => 'address_placeholder,label_search,hide_address_entry'
			);

			$smart_options['hide_radius_selector'] = array(
				'type'       => 'checkbox',
				'default'    => '0',
				'related_to' => 'radii,label_radius,first_entry_for_radius_selector, hide_radius_selector'
			);
		...
		private function general_admin_addons() {
			$smart_options['premium_user_id']         = array( 'call_when_changed' => array( $this, 'trim' ) );
			$smart_options['premium_subscription_id'] = array( 'call_when_changed' => array( $this, 'trim' ) );
		 ...
		 */
		private function general_app_map_services() {
			$smart_options['google_geocode_key'] = array(
				'call_when_changed' => array(
					$this,
					'trim'
				)
			);         // Backend Server Key (geocoding)
			$smart_options['google_server_key']  = array(
				'call_when_changed' => array(
					$this,
					'trim'
				)
			);          // Front End Browser Key
			$this->create_smart_options( $smart_options, array(
				'page'    => 'slp_general',
				'section' => 'server',
				'group'   => 'map_services',
		 ...
		 */
		private function general_app_features() {
			$new_options['force_load_js'] = array(
				'type'        => 'checkbox',
				'default'     => '0',
			...
			$this->create_smart_options( $new_options, array(
				'page'    => 'slp_general',
				'section' => 'server',
		...
		public function get_page_options( $admin_page ) {
			if ( ! isset( $this->page_options[ $admin_page ] ) ) {
				...
				if ( empty( $this->page_layout[ $admin_page ] ) ) {
					$this->page_layout[ $admin_page ] = array();
				}
		...
		public function save( $admin_page = '' ) {
			...
			if ( empty( $admin_page ) ) {
				$admin_page = SLP_Settings::get_instance()->current_admin_page;
			}
		 ...
		 * @return boolean
		 */
		public function set( $property, $value, $writeToDB = false ) {
			$wentOK = false;
			if ( property_exists( $this, $property ) ) {
				if ( $this->$property->use_in_javascript ) {
					$wentOK = $this->set_valid_options( $value, $property ); // this will set the value
				} else {
					$wentOK = $this->set_valid_options_nojs( $value, $property ); // this will set the value
				}
			}
		...
		public function slp_specific_setup() {
			...
			$this->slplus->javascript_is_forced = $this->force_load_js->is_true;
		...
		private function set_the_val( $value, $key, &$option_array, $is_smart_option, $valid_legacy_option ) {
			if ( $is_smart_option && is_object( $this->{$key} ) ) {
				...
				$this->$key->__set( 'value', is_string( $value ) ? stripslashes( $value ) : $value );    // Smart Option objects don't want slashes.
		...
		public function set_valid_options( $val, $key ) {
			return $this->set_valid_option( $val, $key, 'options' );
		}
		...
		public function set_valid_options_nojs( $val, $key ) {
			return $this->set_valid_option( $val, $key, 'options_nojs' );
		}
