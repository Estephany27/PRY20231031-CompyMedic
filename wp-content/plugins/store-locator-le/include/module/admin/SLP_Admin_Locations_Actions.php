...
class SLP_Admin_Locations_Actions extends SLPlus_BaseClass_Object {
	...
	private function add_location() {
		...
		if ( check_admin_referer( 'screen-options-nonce', 'screenoptionnonce' ) ) {
			...
			foreach ( $_POST as $key => $value ) {
				...
				if ( ( $key === 'latitude' ) && ! $this->slplus->currentLocation->is_valid_lat( $value ) ) {
					continue;
				}
				if ( ( $key === 'longitude' ) && ! $this->slplus->currentLocation->is_valid_lng( $value ) ) {
					continue;
				}
	...
	private function save_edited_location() {
		...
		if ( $this->slplus->currentLocation->dataChanged && ! $this->slplus->currentLocation->MakePersistentIfChanged() ) {
		...
		if ( check_admin_referer( 'screen-options-nonce', 'screenoptionnonce' ) ) {
			...
			foreach ( $this->screen->active_columns as $extraColumn ) {
				...
				if ( $extraColumn->type === 'boolean' ) {
					...
					if ( ! isset( $this->slplus->currentLocation->exdata[ $slug ] ) || ( $this->slplus->currentLocation->exdata[ $slug ] !== $new_setting ) ) {
	 ...
	 *
	 * Bulk action: delete is sending $_POST[slp_nonce],  $_POST[_wpnonce], and $_POST['screenoptionnonce']
	 * slp_nonce is the new nonce being pushed by SLP_Settings... may not be needed there.
	 *
	...
	public function set_locations() {
		$this->offset = 0;
		if ( check_admin_referer( 'screen-options-nonce', 'screenoptionnonce' ) ) {
			$this->all = ( ! empty( $_REQUEST['apply_to_all'] ) && ( intval( $_REQUEST['apply_to_all'] ) === 1 ) );
			if ( $this->all ) {
				return true;
			}
			if ( ! empty( $_REQUEST['sl_id'] ) ) {
				$this->locations = array_map( 'sanitize_key', (array) $_REQUEST['sl_id'] );

				return true;
			}
			if ( ! empty( $_REQUEST['id'] ) ) {
				$this->locations = array_map( 'sanitize_key', (array) $_REQUEST['id'] );
