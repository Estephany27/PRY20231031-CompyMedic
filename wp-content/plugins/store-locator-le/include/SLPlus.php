...
if ( ! class_exists( 'SLPlus' ) ) {
	 ...
	 * @property        array $userMetaKeys                   List of valid user meta keys for SLP user meta updates.
	 *
	...
	class SLPlus extends SLP_Object_With_Objects {
		...
		public $url;
		public $url_main_slp_site = 'https://storelocatorplus.com';
		...
		public function __construct() {
			...
			if ( ! SLP_DETECTED_HEARTBEAT ) {
				add_action( 'plugins_loaded', array( $this, 'sanitize_things' ) );
			}
		}
		...
		public function addon( $slug ) {
			switch ( strtolower( $slug ) ) {
				...
				case 'premier':
					$slug = 'slp-premier';
					break;
				// return SLP if no matching plugins are found
				default:
					return $this;
			}
			$addon = $this->AddOns->get( $slug, 'instance' );
		...
		public function find_addon( $class_name ) {
			...
			if ( empty( $this->AddOns->instances ) ) {
				$plugin = $this;
		...
		public function initialize() {
			...
			SLP_Admin_Helper::get_instance();
			SLP_WPOption_Manager::get_instance();
			SLP_SmartOptions::get_instance();
			SLP_AddOns::get_instance();

			$this->notifications = SLP_Notifications_Manager::get_instance(); // alias ->notifications for ->Notifications_Manager
			...
			// -- SLP Objects that need to be initialized
			SLP_REST_Handler::get_instance();
			SLP_Actions::get_instance();
			SLP_UI::get_instance();
		}
		...
		public function initialize_after_plugins_loaded() {
			...
			// TODO drop this when Dashboard and Experience use $this->slplus->Text
			if ( ! isset( $this->WPML ) ) {
				$this->WPML = $this->Text;
			}
		...
		public function enqueue_google_maps_script() {
			if ( empty( $this->SmartOptions->google_server_key->value ) ) {
				return;
			}
			wp_enqueue_script( 'google_maps', $this->get_google_maps_url(), array( 'slp_core' ), SLPLUS_VERSION, ! $this->javascript_is_forced );
		}
		...
		private function get_google_maps_url() {
			...
			// Base Google API URL
			//
			$google_api_url = 'https://maps.googleapis.com/maps/api/js?libraries=geometry&callback=slp.initMap&';

			// Region
			//
			$country_manager = SLP_Country_Manager::get_instance();
			if ( isset( $country_manager->countries[ $this->SmartOptions->default_country->value ] ) ) {
				$country = strtoupper( $country_manager->countries[ $this->SmartOptions->default_country->value ]->cctld );
			} else {
				$country = '';
			}
		...
		public function load_jquery_theme( $theme = 'none' ) {
			...
			if ( is_readable( SLPLUS_PLUGINDIR . $relative_file ) ) {
				wp_enqueue_style( $handle, $this->plugin_url . $relative_file, [], filemtime( SLPLUS_PLUGINDIR . $relative_file ) );
			}
		}
		...
		public function recenter_map() {
			if ( empty( $this->SmartOptions->map_center->value ) ) {
				$this->SmartOptions->set( 'map_center', SLP_Country_Manager::get_instance()->countries[ $this->SmartOptions->default_country->value ]->name );
			}
		 ...
		 */
		public function sanitize_things() {

			// phpcs:ignore non-sanitized is not relevant, the slp_nonce var is only used for hash validation
			// phpcs:ignore
			if ( ! empty( $_REQUEST['slp_nonce'] ) && ! wp_verify_nonce( $_REQUEST['slp_nonce'], 'slp_settings' ) ) {
				return;
			}
			...
			foreach ( $request_params as $key ) {
				$this->clean[ $key ] = ! empty( $_REQUEST[ $key ] ) ? sanitize_key( wp_unslash( $_REQUEST[ $key ] ) ) : '';
			}

			// Int only inputs
			$request_params = array( 'locationID', 'start' );
			foreach ( $request_params as $key ) {
				$this->clean[ $key ] = ! empty( $_REQUEST[ $key ] ) ? intval( wp_unslash( $_REQUEST[ $key ] ) ) : 0;
			}

			// Text only inputs
			$request_params = array( 'selected_nav_element' );
			foreach ( $request_params as $key ) {
				$this->clean[ $key ] = ! empty( $_REQUEST[ $key ] ) ? sanitize_text_field( wp_unslash( $_REQUEST[ $key ] ) ) : '';
			}

			// Orderby inputs
			$request_params = array( 'orderBy' );
			foreach ( $request_params as $key ) {
				$this->clean[ $key ] = ! empty( $_REQUEST[ $key ] ) ? sanitize_sql_orderby( wp_unslash( $_REQUEST[ $key ] ) ) : '';
			}
		...
		private function set_map_center_fallback( $for = 'latlng' ) {
			...
			//
			if ( $this->options['map_center'] == $country_manager->countries[ $this->SmartOptions->default_country->value ]->name ) {

				// Set the default country lat
				//
				if ( ( $for === 'latlng' ) || ( $for === 'lat' ) ) {
					$this->SmartOptions->set( 'map_center_lat', $country_manager->countries[ $this->SmartOptions->default_country->value ]->map_center_lat );
				}

				// Set the default country lng
				//
				if ( ( $for === 'latlng' ) || ( $for === 'lng' ) ) {
					$this->SmartOptions->set( 'map_center_lng ', $country_manager->countries[ $this->SmartOptions->default_country->value ] );
				}
				$this->WPOption_Manager->update_wp_option( 'js' );
				$this->WPOption_Manager->update_wp_option( 'nojs' );
			}
			...
			if ( empty( $this->SmartOptions->map_center_lng->value ) || empty( $this->SmartOptions->map_center_lat->value ) ) {
				$json = SLP_Location_Utilities::get_instance()->geocode( array( 'address' => $this->SmartOptions->map_center->value ) );
				if ( ! empty( $json ) ) {
					if ( is_object( $json ) && ( $json->status === 'OK' ) ) {
						if ( empty( $this->SmartOptions->map_center_lat->value ) ) {
							$this->SmartOptions->set( 'map_center_lat', $json->results[0]->geometry->location->lat );
						}
						if ( empty( $this->SmartOptions->map_center_lng->value ) ) {
							$this->SmartOptions->set( 'map_center_lng', $json->results[0]->geometry->location->lng );
						}
						$this->WPOption_Manager->update_wp_option( 'js' );
						$this->WPOption_Manager->update_wp_option( 'nojs' );
					}
		...
		public function get_apikey() {
			return apply_filters( 'slp_apikey', md5( get_site_url() ) );
		}
		 ...
		 * @return bool
		 */
		public function ua_is_a_bot() {
			$user_agent = ! empty( $_SERVER['HTTP_USER_AGENT'] ) ? sanitize_key( wp_unslash( $_SERVER['HTTP_USER_AGENT'] ) ) : '';

			return ( preg_match( '/bot|googlebot|crawler|spider|robot|crawling/i', $user_agent ) === 1 );
		}
	...
	// phpcs:ignore do not need to process the nonce to short circuit the heartbeat
	// phpcs:ignore
