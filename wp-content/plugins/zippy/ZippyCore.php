...
class ZippyCore
{
    ...
    {
        if (is_bool($options)) {
            _deprecated_argument(__FUNCTION__, '1.5.0', 'Second parameter must be an array of options');
            $options = [
                'replaceExists'  => $options,
                'customPostType' => $deprecated
            ];
        }
        ...
        try {
            ...
            foreach ($postsData as $data) {
                ...
                $postType = empty($options['customPostType'])
                    ? (post_type_exists($post->post_type) ? $post->post_type : '')
                    : $options['customPostType'];
