...
print "<th>(Lat, Lon)</th>
<th colspan='1'>".__("Status", SSF_WP_TEXT_DOMAIN)."</th>
<th colspan='1'>".__("Actions", SSF_WP_TEXT_DOMAIN)."</th>
	...
	if ($locales=$wpdb->get_results("SELECT * FROM ".SSF_WP_TABLE." $where ORDER BY $o $d LIMIT $start, $num_per_page", ARRAY_A)) { 
		if (function_exists("do_ssf_wp_hook") && !empty($ssf_wp_columns)){
			...
			$colspan=($ssf_wp_vars['location_table_view']!="Normal")? 	18 : 12;
		}
...
<td>$value[ssf_wp_image]</td>";
				...
				if($value['ssf_wp_latitude']=="" || !is_numeric($value['ssf_wp_latitude'])){
				$value['ssf_wp_latitude']=0;	
				}
				
				if($value['ssf_wp_longitude']=="" || !is_numeric($value['ssf_wp_longitude'])){
				$value['ssf_wp_longitude']=0;	
				}
				
				$checked_status=($value['ssf_wp_is_published']!="2")? "checked" : '';
				...
				<td><input type='checkbox' class='ssfPublishStatus' onclick='ssfPublishSingle($value[ssf_wp_id])' id='publish_".$value['ssf_wp_id']."' $checked_status ></td>
...
<div id="ssf_wp_publish_success" style="display:none;min-width:500px; background-color:white;     padding: 50px;
font-size: 20px;">
	<div class='ssf_wp_admin_success'>Status changed successfully !</div> 
</div> 
...
<script>
	...
	function ssfUnPublishAll(){
		var ssf_wp_id=jQuery("input[name*='ssf_wp_id']:checked").map(function() {return this.value;}).get().join(',')
		ssfPublishCall(ssf_wp_id, 2);
	}
	
	function ssfPublishAll(){
		var ssf_wp_id=jQuery("input[name*='ssf_wp_id']:checked").map(function() {return this.value;}).get().join(',')
		ssfPublishCall(ssf_wp_id, 1);	
	}
	
	function ssfPublishSingle(id){
		 var checkbox = document.getElementById('publish_'+id);
		 if (checkbox.checked != true){
			var status=2;
		  }else{
			var status=1;
		  }
		ssfPublishCall(id, status);	
	}
	
	function ssfPublishCall(ssf_wp_id, status){
		jQuery.ajax({
				url: ajaxurl, 
				data: {
					'action': 'publish_ssf_request',
					'ssf_wp_id' : ssf_wp_id,
					'publish' : status
				},
				success:function(data) {
					// This outputs the result of the ajax request
					jQuery("#ssf_wp_publish_success").fadeIn(500);
					jQuery("#ssf_wp_publish_success").css({"position": "fixed","top": "10%",
                            "left": "40%",
                            "z-index" : "9999"
                            });
					jQuery("#ssf_wp_publish_success").fadeOut(2000);
