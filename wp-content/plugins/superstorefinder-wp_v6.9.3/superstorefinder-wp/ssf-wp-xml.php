...
foreach ($query as $row) {
  ...
  echo '<custom_field1>' . $row['ssf_wp_custom_field1'] . '</custom_field1>';
  echo '<custom_field2>' . $row['ssf_wp_custom_field2']. '</custom_field2>';
  echo '<custom_field3>' . $row['ssf_wp_custom_field3'] . '</custom_field3>';
  echo '<custom_field4>' . $row['ssf_wp_custom_field4'] . '</custom_field4>';
  echo '<custom_field5>' . $row['ssf_wp_custom_field5'] . '</custom_field5>';
  echo '<custom_field6>' . $row['ssf_wp_custom_field6'] . '</custom_field6>';
  
  echo '<contactus>'.$contactEmail.'</contactus>';
  echo '<storeId>'.$row['ssf_wp_id'].'</storeId>';
  echo '<country>'.ssfParseToXML($row['ssf_wp_country']).'</country>';
 ...
 if($checkAddon==true){ 
  ...
  $tagsList=array();
	for($i=0;$i<sizeof($tagarray)-1;$i++){
		 ...
		 if(!empty($tags)){
			echo '<'.$tags.'>true</'.$tags.'>';
			array_push($tagsList,ssfParseToXML(__($tagarray[$i],','),SSF_WP_TEXT_DOMAIN));
...
echo '<productsServices>'.rtrim($productsServices,',').'</productsServices>';
